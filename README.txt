...
Test class inheritance parallels class inheritance, so although, for example, tests in the TestConcrete modules could be in the TestAbstract modules without changing how they run, the partition reflects the fact that methods implemented in an abstract class are tested in a TestAbstract module while methods implemented in a concrete class are tested in a TestConcrete module. :)
